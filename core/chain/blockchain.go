...
type CacheConfig struct {
	...
	TrieDirtyDisabled   bool          // Whether to disable trie write caching and GC altogether (archive node)是否同时禁用trie写缓存和GC（存档节点）
	TrieTimeLimit       time.Duration // Time limit after which to flush the current in-memory trie to disk
	SnapshotLimit       int           // Memory allowance (MB) to use for caching snapshot entries in memory
	Preimages           bool          // Whether to store preimage of trie key to the disk 是否将trie密钥的前映像存储到磁盘
...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params2.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	root1:= bc.GetHeaderByHash(bc.CurrentBlock().Hash()).Root
	fmt.Println("stateRoot  NewBlockChain",root1.String())
	mystate, _ :=bc.StateAt(root1)
	dump := mystate.RawDump(false, false, true).Accounts
	fmt.Println("len(dump) NewBlockChain ", len(dump))
