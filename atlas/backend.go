...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	fmt.Println("chainConfig, genesisHash, genesisErr", chainConfig, genesisHash.String(), genesisErr)
	...
	//------------------------------------zhangwei-------------------------
	//stateRoot := eth.blockchain.GetHeaderByHash(eth.blockchain.CurrentBlock().Hash()).Root
	//fmt.Println("stateRoot", stateRoot)
	//mystate, err := eth.blockchain.StateAt(stateRoot)
	//if err != nil {
	//	fmt.Println("err zw", err)
	//}
	//dump := mystate.RawDump(false, false, true).Accounts
	//fmt.Println("len(dump)  ", len(dump))
	//------------------------------------zhangwei-------------------------
	...
	fmt.Println("eth.blockchain.CurrentBlock().Hash()", eth.blockchain.CurrentBlock().Hash().String())
	fmt.Println("eth.blockchain.CurrentBlock().Number()", eth.blockchain.CurrentBlock().Number())
	fmt.Println("config.Miner.ExtraData", config.Miner.ExtraData.String())
	//fmt.Println("config.Genesis.Alloc",config.Genesis.Alloc)
	fmt.Println("config.Genesis.Alloc len", len(config.Genesis.Alloc))
	fmt.Println("config.Genesis.Config.ChainID", config.Genesis.Config.ChainID)
	stateRoot := eth.blockchain.GetHeaderByHash(eth.blockchain.CurrentBlock().Hash()).Root
	mystate, err := eth.blockchain.StateAt(stateRoot)
	if err != nil {
	...
	dump := mystate.RawDump(false, false, true).Accounts
	fmt.Println("len(dump)111111111111111111111111111111111111111", len(dump))
	...
	//fmt.Println("genesisAlloc", genesisAlloc) // [] 说明没有初始化到stateDB
	vmRunner := eth.blockchain.NewEVMRunner(eth.blockchain.CurrentHeader(), mystate)
	election.GetElectedValidators(vmRunner)
	//-----------------------------------zhangwei------------------------------------------
